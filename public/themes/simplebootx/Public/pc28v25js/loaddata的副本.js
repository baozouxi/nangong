...
function submitOKtp999(){
	...
	var url,data;
    url="/user/game/game28_buy25_999";
...
function getBetsListCircle(){
	$.ajax({
        url: "/user/game/load_open_bet_number/t/"+Math.random(),
		type: "post",
		data: {
			//"expect": expect
		},
...
function getUserBetsListToday(){
	$.ajax({
        url: "/user/game/getUserBetsListToday_25/t/"+Math.random(),
		type: "post",
		data: {
			//"expect": expect
		},
				...
				$.each(dataJson.userbetList, function(idx, val) {
					...
					html += '<div class="wf">';	
						...
						if (parseInt(val.bettype)<=100) {
							...
							//html += '	<div class="price">' + val.betsMoney + '</div>';
							html += '</div>';
						}else if (parseInt(val.bettype)>=140&&parseInt(val.bettype)<=142) {
							if (parseInt(val.bettype)==140) {ubetname='豹子';}
							if (parseInt(val.bettype)==141) {ubetname='顺子';}
							if (parseInt(val.bettype)==142) {ubetname='对子';}
							html += '<div class="ck_num2">';
							html += '	<div class="quan">' + ubetname + '</div>';
							//html += '	<div class="price">' + val.betsMoney + '</div>';
							html += '</div>';
...
function shownotbet(){
	$.ajax({
        url: "/user/game/getCannotBetType_25/t/"+Math.random(),
		type: "post",
		data: {
			//"expect": expect
		},
...
function loadopentime(){
    ...
    $.ajax({
        type: "post",
        url: "/user/game/load_open_time/t/"+Math.random(),
        data: {
            //"act": "get"
        },
        datatype: "json",
...
function getUserZouShi() {
	$.ajax({
		url: "/getdatav25/loadopenzoushi.asp?act=get&t="+Math.random(),
		type: "post",
		data: {
			//"u": 1
		},
